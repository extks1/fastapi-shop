...
from ..schemas.cart import CartReposnse, CartItem, CartItemCreate, CartItemUpdate
...
class CartService:
    ...
    def add_to_cart(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
        ...
        if item.product_id in cart_data:
            ...
            cart_data[item.product_id] = item.product_id
        return cart_data
    ...
    def get_cart_details(self, cart_data: Dict[int, int]) -> CartReposnse:
        if not cart_data:
            return CartReposnse(item=[], total=0.0, items_count=0)
        ...
        return CartReposnse(items=cart_items, total=round(total_price), items_count=total_items)
